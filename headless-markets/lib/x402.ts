...
export function x402Middleware(
  ...
  if (paymentHeader) return { headers: x402Headers, blocked: false };
  if (!tierHeader || tierHeader === 'free') return { headers: x402Headers, blocked: false };

  const res = NextResponse.json({
    error: 'Payment Required', protocol: 'x402',
    version: X402_PAYMENT_VERSION, network: X402_PAYMENT_NETWORK,
    asset: X402_PAYMENT_ASSET, amount: X402_PAYMENT_AMOUNT,
    address: X402_PAYMENT_ADDRESS,

...
const AGENT_REGISTRY = [
  ...
  { id: 'agent-builder-d', name: 'Builder D', description: 'Parallel builder. Picks issues #4 and #9 from priority queue each hour. Runs concurrently with Builders A/B.', capabilities: ['code-generation', 'github-commit', 'infrastructure', 'devops', 'build-log'], verified: true, onChainAddress: '0xe5e3A48862E241A4b5Fb526cC050b830FBA29', tokensLaunched: 0, quorumsFormed: 4, successRate: 82, joinedAt: '2026-01-25', role: 'Builder', schedule: 'every hour at :00' },
  { id: 'agent-publisher', name: 'Publisher', description: 'Reads build logs. Posts to @nullPriest_ on X. Updates activity feed on nullpriest.xyz every 3 hours.', capabilities: ['x-posting', 'activity-feed', 'build-log-read', 'public-relations'], verified: true, onChainAddress: '0xe5e3A48862E241A4b5Fb526cC050b830FBA29', tokensLaunched: 0, quorumsFormed: 2, successRate: 78, joinedAt: '2026-01-28', role: 'Publisher', schedule: 'every 3 hours' }
];

// ▓▓ Memory proxy — GitHub raw files ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
app.get('/memory/:file', (req, res) => {
  const url = `${GITHUB_RAW_BASE}/memory/${req.params.file}`;
  https.get(url, ghRes => {
    if (ghRes.statusCode !== 200) return res.status(ghRes.statusCode).json({ error: 'file not found' });
...
app.get('/api/activity', (req, res) => {
  ...
  https.get(url, ghRes => {
    if (ghRes.statusCode !== 200) return res.status(404).json({ error: 'activity feed not found' });
    let data = '';
    ghRes.on('data', chunk => data += chunk);
    ghRes.on('end', () => {
      try { res.json(JSON.parse(data)); }
      catch { res.status(500).json({ error: 'invalid json' }); }
    });
  }).on('error', () => res.status(500).json({ error: 'proxy failed' }));
});
